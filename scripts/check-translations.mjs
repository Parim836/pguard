...
try {
  const { LanguageProvider } = await server.ssrLoadModule("/src/LanguageContext.tsx");
  const { default: Navbar } = await server.ssrLoadModule("/src/components/Navbar.tsx");
  const { default: Footer } = await server.ssrLoadModule("/src/components/Footer.tsx");
  const cases = {
    Home: ["เรียกเจ้าหน้าที่รักษาความปลอดภัยมืออาชีพ", "Book professional security guards"],
    ...
    Documents: ["เอกสารสำหรับสมัครเป็น รปภ.", "Documents for security guard registration"],
    ...
    for (const [index, language] of ["TH", "EN"].entries()) {
      ...
      const markup = renderToStaticMarkup(createElement(MemoryRouter, null,
        createElement(LanguageProvider, null,
          createElement(Navbar), createElement(Page), createElement(Footer))));
      assert.ok(markup.includes(headings[index]), `${name}: missing ${language} heading`);
      if (language === "EN") {
        assert.doesNotMatch(markup, /[\u0e01-\u0e3a\u0e40-\u0e5b]/, `${name}: untranslated Thai text`);
      }
    }
    console.log(`${name}: Thai and English rendering passed`);
  }
  const { default: Features } = await server.ssrLoadModule("/src/pages/Features.tsx");
  for (const value of [null, "unsupported", "blocked"]) {
    Object.defineProperty(globalThis, "localStorage", {
      configurable: true,
      value: { getItem() { if (value === "blocked") throw new Error("Storage blocked"); return value; } },
    });
    const markup = renderToStaticMarkup(createElement(LanguageProvider, null, createElement(Features)));
    assert.ok(markup.includes("ฟีเจอร์ของ PGUARD"), "Storage fallback should be Thai");
  }
